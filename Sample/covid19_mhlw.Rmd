...
### 重症者数
```{r}
si_mhlw <- "https://www.mhlw.go.jp/content/severe_daily.csv" %>% 
  readr::read_csv() %>% 
  dplyr::mutate(date = lubridate::as_date(`日付`),
                si = as.integer(`重症者数`)) %>% 
...
x_mhlw <- df_mhlw %>% 
  ...
  dplyr::left_join(si_mhlw, by = c("date")) %>% 
  dplyr::select(date, n, diff, cum, ma7, dead, patient, pcr, si) %>% 
  tidyr::replace_na(list(dead = 0L, patient = 0L, pcr = 0L, si = 0L)) %>% 
  dplyr::mutate(diff_dead = lagdiff(dead), cum_dead = cumsum(dead),
                ...
                ma7_pcr = ma7(pcr),
                diff_si = lagdiff(si), cum_si = cumsum(si),
                ma7_si = ma7(si)) %>% 
  dplyr::select(date, n, diff, cum, ma7, patient, diff_patient,
                dead, diff_dead, cum_dead, ma7_dead,
                pcr, diff_pcr, cum_pcr, ma7_pcr, si, diff_si, cum_si, ma7_si)
x_mhlw
```
...
x_mhlw %>% 
  ggplot2::ggplot(ggplot2::aes(x = date)) + 
    ggplot2::geom_bar(ggplot2::aes(y = si), stat = "identity",
                      fill = "dark gray", width = 1.0, alpha = 1.0) + 
    ggplot2::geom_line(ggplot2::aes(y = ma7_si), colour = "dark green",
                       linetype = "dashed", size = 0.2) + 
    ggplot2::geom_line(ggplot2::aes(y = cum_si / scale_axis),
                       colour = "dark green", size = 1.0) +
    ggplot2::labs(title = paste0("全国の重症者数の推移（単日） @", datetime),
                  subtitle = subtitle, caption = caption,
                  x = "", y = "") + 
    ggplot2::scale_y_continuous(
      name = "重症者数・同移動平均（破線）",
...
x_mhlw %>% 
  ...
  ggplot2::ggplot(ggplot2::aes(x = date)) + 
    ggplot2::geom_bar(ggplot2::aes(y = pcr / scale_axis), stat = "identity",
                      width = 1.0, alpha = 0.25, fill = "dark blue") +
    ggplot2::geom_bar(ggplot2::aes(y = n), stat = "identity",
                      width = 1.0, alpha = 0.25, fill = "dark green") +
