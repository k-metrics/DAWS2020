...
[COVID-19 対策ダッシュボード](https://covid-19.nec-solutioninnovators.com/) | 公開 | 集計 | CSV | 日英 | 可 | NECソリューションイノベーター
...
NECソリューションイノベータによる[tableau](https://public.tableau.com/profile/nec.solution.innovators.ltd.#!/vizhome/COVID-19_15903749159280/sheet1)を使ったダッシュボード。
...
エラーにはなりませんが、各列のデータ型を正しく推定するためには`readr::read_csv`関数で`guess_max = 12500`オプションを指定してください。  

なお、ファイルのエンコーディングは`UTF-8 with BOM`（ExcelのUTF-8エンコーディングCSV形式）になっています。
