...
x_prefs_ma <- x_prefs %>% 
  tidyr::pivot_wider(names_from = Pref, values_from = n, values_fill = 0L) %>% 
  dplyr::mutate_if(is.integer, .funs = ma7) %>% 
  tidyr::pivot_longer(cols = -date, names_to = "Pref", values_to = "ma7")
x_prefs_ma  
...
x_by_prefs <- x_prefs %>% 
  ...
  dplyr::left_join(x_prefs_ma, by = c("date" = "date", "Pref" = "Pref")) %>% 
...
d_by_prefs <- d_prefs %>% 
  dplyr::left_join(d_prefs_diff, by = c("date" = "date", "Pref" = "Pref")) %>% 
  dplyr::left_join(d_prefs_cum, by = c("date" = "date", "Pref" = "Pref")) %>% 
  dplyr::left_join(d_prefs_ma, by = c("date" = "date", "Pref" = "Pref")) %>% 
  dplyr::left_join(prefs, ., by = c("都道府県" = "Pref")) %>% 
  dplyr::mutate(Pref = forcats::fct_inorder(`都道府県`)) %>% 
  dplyr::select(date, Pref, n, diff, cum, ma7) %>% 
  dplyr::arrange(date)
...
d_by_prefs %>% 
  dplyr::select(date, Pref, n) %>% 
  tidyr::pivot_wider(names_from = Pref, values_from = n) %>% 
  dplyr::summarise_if(is.integer, .funs = sum) %>% 
  tibble::rownames_to_column("id") %>% 
  tidyr::pivot_longer(cols = -id, names_to = "Pref", values_to ="deceased") %>% 
  dplyr::select(-id)
```
