...
[Covid19 Japan](https://covid19japan.com)が独自に収集している陽性者単位のデータ（個票データ）。ソースとデータは全て[GitHub](https://github.com/reustle/covid19japan)にて公開されており、データはJSON形式。「レコード数 $\neq$ 累計陽性者数」であることに注意。
...
x_by_all %>% 
  ...
  ggplot2::ggplot(ggplot2::aes(x = date)) + 
    ...
    ggplot2::geom_line(ggplot2::aes(y = ma7.x),
                       colour = "dark green", linetype = "dashed") +
    ggplot2::geom_line(ggplot2::aes(y = ma7.y / sec_scale),
                       colour = "dark red", linetype = "dashed") +
    ggplot2::labs(title = paste0("@", datetime), caption = caption,
                  x = "", y = "") + 
    ggplot2::scale_y_continuous(
      name = "陽性者数（濃緑）・移動平均（破線）",
      sec.axis = ggplot2::sec_axis(~ . * sec_scale,
                                    name = "死亡者数（濃赤）・移動平均（破線）")
    )
```
...
x_by_region %>% 
  ...
  ggplot2::ggplot(ggplot2::aes(x = date)) + 
    ...
    ggplot2::geom_bar(ggplot2::aes(y = n.y / sec_scale), stat = "identity",
                      fill = "dark red", alpha = 0.25, width = 1.0) +
    ggplot2::geom_line(ggplot2::aes(y = ma7.x),
                       colour = "dark green", linetype = "dashed", size = 0.25) +
    ggplot2::geom_line(ggplot2::aes(y = ma7.y / sec_scale),
                       colour = "dark red", linetype = "dashed", size = 0.25) +
    ggplot2::facet_wrap(~ region, ncol = ncol, scales = "free_y") + 
    ggplot2::labs(title = paste0("Free Y scale @", datetime), caption = caption,
                  x = "", y = "") + 
    ggplot2::scale_y_continuous(
