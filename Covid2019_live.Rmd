...
caption <- "Data Source: 2019ncovjapan.live"
```


[Covid2019.live](https://covid-2019.live/)は、Rと[Shiny](https://shiny.rstudio.com/)で構築されたダッシュボード型のサイト。データは自動で収集・処理しているようで[GitHub](https://github.com/swsoyee/2019-ncov-japan)にて処理コードも含めて公開されている。  
なお、サーバの処理能力が低いようで負荷がかかると表示できなくなる。  

　  

# Import
[GitHub](https://github.com/swsoyee/2019-ncov-japan)では様々なデータが公開されているが、基本的にはCSV形式になっている。


## 集計データ
陽性者単位の個票データ。
```{r}
"https://raw.githubusercontent.com/swsoyee/2019-ncov-japan/master/50_Data/covid19_jp.csv" %>% 
  readr::read_csv()
```
...
"https://docs.google.com/spreadsheets/d/10MFfRQTblbOpuvOs_yjIYgntpMGBg592dL8veXoPpp4" %>% 
...
```{r, include=FALSE, eval=FALSE}
# 作業用シート
"https://docs.google.com/spreadsheets/d/1Ot0T8_YZ2Q0dORnKEhcUmuYCqZ1y81PIsIAMB7WZE8g" %>% 
  googlesheets4::read_sheet(sheet = "罹患者_東京", col_types = "c") %>% 
  readr::write_csv(paste0("./Covid19/SIGNATE_CaseData_Tokyo_", lubridate::today(), ".csv"))
```

```{r, include=FALSE, eval=FALSE}
# 作業用シート
"https://docs.google.com/spreadsheets/d/1ji8bheTyfDok1fTImGA9z5qrHvd6TS73HEfKVJyi9Oo" %>% 
  googlesheets4::read_sheet(sheet = "罹患者_大阪", col_types = "c") %>% 
  readr::write_csv(paste0("./Covid19/SIGNATE_CaseData_Osaka_", lubridate::today(), ".csv"))
```

```{r, include=FALSE, eval=FALSE}
# 作業用シート
"https://docs.google.com/spreadsheets/d/1CJsegQECPiGk2mTAnvpsbrDRkmoqlFUBmA13cPiBVNs" %>% 
  googlesheets4::read_sheet(sheet = "罹患者_東京・大阪以外", col_types = "c") %>% 
  readr::write_csv(paste0("SIGNATE_CaseData_Others_", lubridate::today(), ".csv"))
```
