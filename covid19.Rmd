...
## 厚生労働省
厚生労働省の[オープンデータ](https://www.mhlw.go.jp/stf/covid-19/open-data.html)のページから取得した「陽性確定者数」のデータ。
```{r}
...
df
# df %>% readr::write_excel_csv("./Covid19/covid19japan.csv")
```
...
## ソースによる違い
```{r, echo=FALSE}
n_mhlw <- df_mhlw %>% with(max(cum))
d_mhlw <- dead_mhlw %>% with(max(cum_dead))

date_mhlw <- df_mhlw %>% 
  with(max(date))

n_c19j <- df %>% 
  dplyr::filter(patientId != "-1") %>% nrow()
d_c19j <- df %>% 
  dplyr::filter(patientStatus == "Deceased") %>% nrow()

date_c19j <- df %>% 
  with(max(dateAnnounced))

n_jag <- df_jag %>% nrow()

date_jag <- df_jag %>% 
  dplyr::mutate(`確定日` = lubridate::mdy(`確定日`)) %>% 
  with(max(`確定日`))
```
...
--------------|--------------|------------|-----
厚生労働省    | `r n_mhlw`   | `r d_mhlw` | `r date_mhlw`
Covid19 Japan | `r n_c19j`   | `r d_c19j` | `r date_c19j`
JAG Japan     | `r n_jag`    | N/A        | `r date_jag`
NHK（参考）   | 98993        | 1738       | 2020-10-28
...
```{r}
df %>% 
  dplyr::group_by(detectedPrefecture) %>% 
  dplyr::summarise(n = n())
```
...
```{r}
x <- df %>% 
  dplyr::filter(confirmedPatient == TRUE) %>% print() %>% 
  ...
  dplyr::select(patientId, dateAnnounced, ageBracket, gender, detectedPrefecture,
