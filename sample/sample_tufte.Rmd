...
link-citations: yes
# Additional setting for Japanese fonts
header-includes: |
  \usepackage{zxjatype}
...
knitr::opts_chunk$set(cache.extra = packageVersion('tufte'),
                      ...
                      dev = "cairo_pdf", dev.args = list(family = "Noto Serif"))
...
## クロス集計
　都道府県ごとの日次集計、年代別の日次集計など複数の変数の水準ごとの集計をクロス集計と呼びます。このクロス集計も単純集計と同様に `dplyr::group_by` と `dplyr::summrize(n =)`^[単純集計と同様に `dplyr::count` を用いても同様の結果を得ることができます。] で計数することができます。  
...
> 上グラフは `ggthemes::theme_tufte` を適用し Tufte スタイルで描画しています。右側のデフォルトテーマ（`ggplot2::theme_gray`）で描画したグラフと比べるとインクレシオが低いことが分かります。
